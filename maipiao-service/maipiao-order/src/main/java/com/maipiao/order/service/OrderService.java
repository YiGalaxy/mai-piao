...
public class OrderService {
    ...
    private Order buildOrder(OrderDtos.CreateOrderRequest request, Long userId,
        ...
        order.setFilmId(toLong(schedule.get("filmId")));
        order.setFilmName(str(schedule.get("filmName")));
        order.setCinemaId(toLong(schedule.get("cinemaId")));
        order.setCinemaName(str(schedule.get("cinemaName")));
        order.setHallName(str(schedule.get("hallName")));
    ...
    private List<OrderItem> buildItems(Order order, OrderDtos.CreateOrderRequest request) {
        ...
        for (int i = 0; i < request.seatIndexes().size(); i++) {
            ...
            // ledger's unique key is (schedule_id, seat_id), so it has to be set.
